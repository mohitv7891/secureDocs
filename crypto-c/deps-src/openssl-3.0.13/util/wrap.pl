...
BEGIN {
    ...
    require '/miniProject/dev/crypto-wasm-test/deps-src/openssl-3.0.13/util/perl/OpenSSL/Util.pm';
    OpenSSL::Util->import();
}
...
if ($ARGV[0] eq '-fips') {
    $std_openssl_conf = '/miniProject/dev/crypto-wasm-test/deps-src/openssl-3.0.13/test/fips-and-base.cnf';
    shift;
